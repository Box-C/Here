...
if (life_pulloutallplayers > _kGesamt) exitWith {
	life_pulloutallplayers = life_pulloutallplayers - _kGesamt;
...
if (life_getinvehicle > _kGesamt) exitWith {// Bezahlen
	hint "You do not have enough cash. Check your bank account.";
	life_getinvehicle = life_getinvehicle - _kGesamt;
	[6] call SOCK_fnc_updatePartial;
...
format["%1 (%2) Can not pay the bill.\nEs not enough in their bank $ %3 or in thier hands $ %4",profileName,getPlayerUID player,_kGesamt - life_getinvehicle,_kGesamt - life_pulloutallplayers] remoteExecCall ["hint",_taxiFahrer];