...
VERSION_DATE_IS = '02-07-2017 01-59-25#5441';
infiSTAR_MAIN_CODE = "
...
fnc_LBDblClick_RIGHT = {
	...
	switch (_click) do {
		...
		case 'clear ingame HackLog':{};
		case 'clear ingame AdminLog':{};
...
fnc_Altislife_money = {
	...
	if(_state == 'cash')then
	{
		life_pulloutallplayers = life_pulloutallplayers + _amount;
		_log = format['Cash %1K - %2K has been added.',life_pulloutallplayers/1000,_amount/1000];
		_log call FN_SHOW_LOG;
	};
	if(_state == 'bank')then
	{
...
infiSTAR_A3Heal = {
	if(!isNil 'life_pulloutallplayers')then{life_thirst = 100;life_hunger = 100;};
	if(infiSTAR_MOD == 'Epoch')then{
