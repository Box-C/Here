...
if (side _killer isEqualTo west && !(playerSide isEqualTo west)) then {
    ...
    if (!life_use_atm && {getInVehicles > 0}) then {
        [format [localize "STR_Cop_RobberDead",[getInVehicles] call life_fnc_numberText]] remoteExecCall ["life_fnc_broadcast",RCLIENT];
        getInVehicles = 0;
    };
};
