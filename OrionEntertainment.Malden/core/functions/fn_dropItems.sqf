    ...
    switch (_item) do {
        ...
        case "life_pulloutallplayers": {
            if (getInVehicles > 0) then {
                ...
                [_obj] remoteExecCall ["life_fnc_simDisable",RANY];
				_obj setVariable["CurrentTime",serverTime];
                missionNamespace setVariable ["getInVehicles",0];
            };
        };
...
} forEach (("true" configClasses (missionConfigFile >> "VirtualItems")) + ["life_pulloutallplayers"]);
