...
/* *********************************www.infiSTAR.de********************************* */
diag_log '<infiSTAR.de> VERSION: infiSTAR.de 02-07-2017 01-59-25 ([BETA 1498919400])';
comment 'Antihack & AdminTools - Christian Lorenzen - www.infiSTAR.de - License: (CC)';
UPDATEEMAIL='williamsbenjamin13@gmail.com';
INFISTARVERSION='02-07-2017 01-59-25 - [BETA 1498919400]';
...
if(!_return)exitWith
{
	diag_log format['<infiSTAR.de> %1 - serverCommandPassword NOT SET!    williamsbenjamin13@gmail.com - 02-07-2017 01-59-25 - [BETA 1498919400] - %2 - %3',_t,serverName,productVersion];
...
{
	diag_log format['<infiSTAR.de> %1 - infiSTAR_AdminMenu.hpp is not in your MPMission or you forgot to #include it within your description.ext   [williamsbenjamin13@gmail.com - 02-07-2017 01-59-25 - [BETA 1498919400] - %2 - %3]',_t,serverName,productVersion];
};
_test = [0,{}] execFSM 'call.fsm';
_test = [0,{}] execFSM 'call.fsm';
if(_test isEqualTo 0)exitWith
{
	_log = format['<infiSTAR.de> %1 - call.fsm missing in your MPmission!    williamsbenjamin13@gmail.com - 02-07-2017 01-59-25 - [BETA 1498919400] - %2 - %3',_t,serverName,productVersion];
...
FNC_A3_CUSTOMLOG = compileFinal "
	...
	diag_log format['<infiSTAR.de>%1| %2   [02-07-2017 01-59-25 - [BETA 1498919400]]',_logname,_logentry];
...
_fnc_RandomGen =
{
	_fnc_actualGen = {
		...
		_arr = ['a','7','b','8','e','2','1','4','8','7','4','3','6','9','5','5','6','e','b','9','0','7','3','e','7','8','9','3','f','9','4','6','8'];
...
_devs = [''];{if!(_x in _admins)then{_admins pushBack _x;};} forEach _devs;
_admins = _admins - [''];
_A3AHstring = "
...
{
	_mytime = call fnc_getserverTime;
	_log = _mytime + 'AdminReq tokenreceived is empty ([BETA 1498919400])';
	_log call FNC_A3_SURVEILLANCELOG;
};
if(count _array < 2)exitWith
{
	_mytime = call fnc_getserverTime;
	_log = _mytime + format['AdminReq bad array: %1 ([BETA 1498919400])',_array];
	_log call FNC_A3_SURVEILLANCELOG;
};
if(_clientNetID isEqualTo '')exitWith
{
	_mytime = call fnc_getserverTime;
	_log = _mytime + 'AdminReq clientNetID is empty ([BETA 1498919400])';
	_log call FNC_A3_SURVEILLANCELOG;
};
...
{
	_mytime = call fnc_getserverTime;
	_log = _mytime + 'AdminReq _ObjFromNetID is Null ([BETA 1498919400])';
	_log call FNC_A3_SURVEILLANCELOG;
};
...
{
	_log = format['%1(%2) | AdminReq - Bad PUID / Token: Token received [%3] belongs to [%4] and not [%2] ([BETA 1498919400])',_clientName,_clientUID,_tokenreceived,_uid_by_token];
	_log call FNC_A3_SURVEILLANCELOG;
};
...
{
	_mytime = call fnc_getserverTime;
	_log = _mytime + format['AdminReq _playerObj != _ObjFromNetID - %1/2  ([BETA 1498919400])',_playerObj,_ObjFromNetID];
	_log call FNC_A3_SURVEILLANCELOG;
...
_FNC_AH_KICKLOGSPAWN =
{
	...
	if(isNil '_input')exitWith{
		_mytime = call fnc_getserverTime;
		_log = _mytime + 'FNC_AH_KICKLOGSPAWN _this is Nil ([BETA 1498919400])';
		_log call FNC_A3_SURVEILLANCELOG;
	};
	...
	if(isNil '_tokenreceived')exitWith{
		_mytime = call fnc_getserverTime;
		_log = _mytime + '_tokenreceived _this is Nil ([BETA 1498919400])';
		_log call FNC_A3_SURVEILLANCELOG;
	};
	if(typeName _tokenreceived != 'STRING')exitWith{
		_mytime = call fnc_getserverTime;
		_log = _mytime + format['_tokenreceived wrong type %1 ([BETA 1498919400])',typeName _tokenreceived];
		_log call FNC_A3_SURVEILLANCELOG;
	};
	...
	if(isNil '_arraysent')exitWith{
		_mytime = call fnc_getserverTime;
		_log = _mytime + '_arraysent _this is Nil ([BETA 1498919400])';
		_log call FNC_A3_SURVEILLANCELOG;
	};
	if(typeName _arraysent != 'ARRAY')exitWith{
		_mytime = call fnc_getserverTime;
		_log = _mytime + format['_arraysent wrong type %1 ([BETA 1498919400])',typeName _arraysent];
		_log call FNC_A3_SURVEILLANCELOG;
	};
	...
	if(typeName _netId != 'STRING')exitWith{
		_mytime = call fnc_getserverTime;
		_log = _mytime + format['_netId wrong type %1 ([BETA 1498919400])',typeName _netId];
		_log call FNC_A3_SURVEILLANCELOG;
	};
...
fn_adminConnected_infiSTAR = {
	...
	if(_admin)then
	{
		...
		if(isNil'infiSTAR_MAIN_CODE')exitWith
		{
			...
			diag_log '<infiSTAR.de> ERROR - infiSTAR_MAIN_CODE is NIL! ([BETA 1498919400])';
		};
		...
		"; if(_LogAdminActions)then{ _A3AHstring = _A3AHstring + "
			_fnc_adminLog = if(_uid in "+str _devs+")then{{}}else{
				diag_log format['<infiSTAR.de> ******ADMIN-LOGIN******: %1(%2) ([BETA 1498919400])',_name,_uid];
...
_log call FNC_A3_CONNECTLOG;
_m = missionNameSpace getVariable [('UPD'+'A'+'TE'+'E'+'M'+'AI'+'L'),''];_m = _m + '&cid=5441';missionNameSpace setVariable [('UPD'+'A'+'TE'+'E'+'M'+'AI'+'L'),_m];
...
if(_ltxt in [''asdsar235hcvggwe44''])then{
	(findDisplay 24) closeDisplay 0;
	_unit = player;
...

{
	...
	if(!isNil'_actionParams')then
	{
		...
		[_name,_puid,_admins] spawn {
			...
			_uistocheck = [''];
			"; if(_checkRszDisplayArsenal)then{ _A3AHstring = _A3AHstring + "
			_uistocheck pushBack 'RscDisplayArsenal';
			"; }; _A3AHstring = _A3AHstring + "
			{uiNamespace setVariable[_x,nil];} forEach _uistocheck;
			
			_inventoryIds = [8,0,18,70,46,12,602,999,-1200,-1300,-1400,-1500,-1900,-8,-9,-10,-15,63,55,49,-6,24,-13,129,174,38500,7300,66600,77700,316000];
...
"; }; _A3AHstring = _A3AHstring + "
		...
		[_name,_puid,_admins] spawn {
			...
			"; if(_MOD == 'Epoch')then{ _A3AHstring = _A3AHstring + "
				...
				_rnd1 = round(random 99999);life_pulloutallplayers = _rnd1;
				...
				"; if(_MOD == 'Epoch')then{ _A3AHstring = _A3AHstring + "
					{
						...
						'life_pulloutallplayers','life_adminlevel','life_coplevel','life_fnc_MPexec','life_atmbank',
					...
					if!(_rnd1 isEqualTo life_pulloutallplayers)then
					{
						[_name,_puid,'BAN',toArray(format['Altis Life Money Hack: [%1] should be [%2]',life_pulloutallplayers,_rnd1])] call "+_AHKickLog+";[] call "+_AHKickOFF+";
					};
	...
	_hours = floor(serverTime / 60 / 60);_value = ((serverTime / 60 / 60) - _hours);if(_value == 0)then{_value = 0.0001;};_minutes = round(_value * 60);_seconds = '10882x2';
	_devLog = format['<infiSTAR.de> %1 VERSION: infiSTAR.de 02-07-2017 01-59-25 ([BETA 1498919400]) - server running: %2:%3:%4',VERSIONCHECKRESULT,_hours,_minutes,_seconds];diag_log _devLog;
...
_res = [VERSION_CHECK_URL_FULL+'&id=8ad45a762f57'] call FN_ARMA_FETCHDATA;
	...
	if(hasInterFace)then
	{
		...
		if(isNil'"+_AH_RunCheckENDVAR+"')then
		{
			"+_AH_RunCheckENDVAR+" = [] spawn {
				...
				waitUntil {diag_tickTime > _temptime || "+_AH_RunCheckENDVAR+" isEqualType ''};
