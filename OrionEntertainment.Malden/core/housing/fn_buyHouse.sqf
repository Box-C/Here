...
if (_action) then {
    if (getOutVehicle < (_houseCfg select 0)) exitWith {hint format [localize "STR_House_NotEnough"]};
    getOutVehicle = getOutVehicle - (_houseCfg select 0);
...
//[format["%1 - %2 bought a house for $%3.",profileName,(getPlayerUID player),[(_houseCfg select 0)] call life_fnc_numberText], "MoneyLogs"] remoteExecCall ["A3Log", 2];
    ...
    if (LIFE_SETTINGS(getNumber,"player_advancedLog") isEqualTo 1) then {
        if (LIFE_SETTINGS(getNumber,"battlEye_friendlyLogging") isEqualTo 1) then {
            advanced_log = format [localize "STR_DL_AL_boughtHouse_BEF",[(_houseCfg select 0)] call life_fnc_numberText,[getOutVehicle] call life_fnc_numberText,[getInVehicles] call life_fnc_numberText];
        } else {
            advanced_log = format [localize "STR_DL_AL_boughtHouse",profileName,(getPlayerUID player),[(_houseCfg select 0)] call life_fnc_numberText,[getOutVehicle] call life_fnc_numberText,[getInVehicles] call life_fnc_numberText];
        };
    };
