...
if (_hasLicense) then {
    ...
    if (getInVehicles < _cost) exitWith {hint format [localize "STR_Process_License",[_cost] call life_fnc_numberText]; "progressBar" cutText ["","PLAIN"]; life_is_processing = false; life_action_inUse = false;};
    ...
    "progressBar" cutText ["","PLAIN"];
    if (_minimumConversions isEqualTo (_totalConversions call BIS_fnc_lowestNum)) then {[localize "STR_NOTF_ItemProcess",true,"slow"] call life_fnc_notificationSystem;} else {[localize "STR_Process_Partial",true,"slow"] call life_fnc_notificationSystem;};
    getInVehicles = getInVehicles - _cost;
