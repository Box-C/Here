...
if ([true,_type,_amount] call life_fnc_handleInv) then {
	if (!isNil "_hideout" && {!isNil {group player getVariable "gang_bank"}} && {(group player getVariable "gang_bank") >= _price}) then {
		...
		if (_action) then {
			...
			try {
				if (getInVehicles < _realPrice) then {
					if (getOutVehicle < _realPrice) then {
						[localize "STR_NOTF_NotEnoughMoney",true,"slow"] call life_fnc_notificationSystem; [false,_type,_amount] call life_fnc_handleInv;
						throw "No money";
					};
				};
				
				if ((getOutVehicle > _realPrice) && (getInVehicles < _realPrice)) then {
					if (LIFE_SETTINGS(getNumber,"debit_tax") isEqualTo 1) then {
						...
						getOutVehicle = getOutVehicle - _realPrice;
						...
						if (life_has_debit isEqualTo true) then {
							...
							getInVehicles = getInVehicles - _realPrice;
