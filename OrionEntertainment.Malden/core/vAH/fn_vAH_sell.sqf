...
if(_action) then {
		if(getOutVehicle < _tax) exitWith {["You don't have enough money in the bank to pay the tax",false,"slow"] call life_fnc_notificationSystem;_bad = true;};
	...
	switch (_type) do 
		...
		getOutVehicle = getOutVehicle - _tax;
		closeDialog 0;
