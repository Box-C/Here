...
if (_value > getOutVehicle) exitWith {hint localize "STR_ATM_NotEnough"};
_tax = _value * LIFE_SETTINGS(getNumber,"bank_transferTax");
if ((_value + _tax) > getOutVehicle) exitWith {hint format[localize "STR_ATM_SentMoneyFail",_value,_tax]};

getOutVehicle = getOutVehicle - (_value + _tax);
...
if (LIFE_SETTINGS(getNumber,"player_moneyLog") isEqualTo 1) then {
    if (LIFE_SETTINGS(getNumber,"battlEye_friendlyLogging") isEqualTo 1) then {
        money_log = format ["transferred $%1 to %2. Bank Balance: $%3  On Hand Balance: $%4",_value,_unit getVariable ["realname",name _unit],[getOutVehicle] call life_fnc_numberText,[getInVehicles] call life_fnc_numberText];
    } else {
        money_log = format ["%1 - %2 transferred $%3 to %4. Bank Balance: $%5  On Hand Balance: $%6",profileName,(getPlayerUID player),_value,_unit getVariable ["realname",name _unit],[getOutVehicle] call life_fnc_numberText,[getInVehicles] call life_fnc_numberText];
    };
