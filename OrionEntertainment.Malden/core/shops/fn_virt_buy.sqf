...
if ((_price * _amount) > getInVehicles && {!isNil "_hideout" && {!isNil {group player getVariable "gang_bank"}} && {(group player getVariable "gang_bank") <= _price * _amount}}) exitWith {hint localize "STR_NOTF_NotEnoughMoney"};
    ...
    if (!isNil "_hideout" && {!isNil {group player getVariable "gang_bank"}} && {(group player getVariable "gang_bank") >= _price}) then {
        _action = [
            format[(localize "STR_Shop_Virt_Gang_FundsMSG")+ "<br/><br/>" +(localize "STR_Shop_Virt_Gang_Funds")+ " <t color='#8cff9b'>%1$</t><br/>" +(localize "STR_Shop_Virt_YourFunds")+ " <t color='#8cff9b'>%2$</t>",
                ...
                [getInVehicles] call life_fnc_numberText
            ],
        ...
        if (_action) then {
            ...
            if ((_price * _amount) > getInVehicles) exitWith {[false,_type,_amount] call life_fnc_handleInv; [localize "STR_NOTF_NotEnoughMoney",true,"slow"] call life_fnc_notificationSystem;};
            ...
            getInVehicles = getInVehicles - _price * _amount;
        };
    } else {
        if ((_price * _amount) > getInVehicles) exitWith {[localize "STR_NOTF_NotEnoughMoney",true,"slow"] call life_fnc_notificationSystem; [false,_type,_amount] call life_fnc_handleInv;};
        hint format[localize "STR_Shop_Virt_BoughtItem",_amount,(localize _name),[(_price * _amount)] call life_fnc_numberText];
        [player,"buy"] remoteexeccall ["say3D",0];
        getInVehicles = getInVehicles - _price * _amount;
    };
