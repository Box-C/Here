...
if (parseNumber(_amount) > getInVehicles) exitWith {[localize "STR_NOTF_notEnoughtToGive",true,"slow"] call life_fnc_notificationSystem;ctrlShow[2001,true];};
if (isNull _unit) exitWith {ctrlShow[2001,true];};
...
getInVehicles = getInVehicles - (parseNumber(_amount));
