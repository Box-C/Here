...
if (!isNull _source) then {
    if (_source != _unit) then {
        if (currentWeapon _source in ["hgun_Pistol_heavy_01_F","arifle_MXC_Black_F","arifle_MX_Black_F","arifle_MXM_Black_F","arifle_Katiba_F","arifle_AKM_F","arifle_CTAR_blk_F","arifle_SPAR_01_blk_F","SMG_02_F","srifle_DMR_07_ghex_F"] && currentMagazine _source in ["20Rnd_650x39_Cased_Mag_F","30Rnd_9x21_Red_Mag","30Rnd_556x45_Stanag_green","30Rnd_580x42_Mag_Tracer_F","30Rnd_65x39_caseless_mag_Tracer","11Rnd_45ACP_Mag","30Rnd_762x39_Mag_F","30Rnd_65x39_caseless_green"]) then {
            if (side _source isEqualTo west && playerSide isEqualTo civilian) then {
                _damage = 0;
                if (alive player && !life_istazed && !life_isknocked && !(_unit getVariable ["restrained",false])) then {
                    private ["_distance"];
                    _distance = 100;
                    if (currentMagazine _source in ["30Rnd_65x39_caseless_mag_Tracer","30Rnd_762x39_Mag_F","30Rnd_65x39_caseless_green"]) then {_distance = 600;};
