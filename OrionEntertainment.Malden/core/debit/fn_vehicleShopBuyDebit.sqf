...
try {
	if (getInVehicles < _purchasePrice) then {
		if (getOutVehicle < _purchasePrice) then {
			hint parseText format [localize "STR_Shop_Veh_NotEnough",[_purchasePrice - getOutVehicle] call life_fnc_numberText];
			throw "No money";
		};
	};
			
	if ((getOutVehicle > _purchasePrice) && (getInVehicles < _purchasePrice)) then {
		if (LIFE_SETTINGS(getNumber,"debit_tax") isEqualTo 1) then {
			...
			getOutVehicle = getOutVehicle - _priceAfterTax;
			...
			getOutVehicle = getOutVehicle - _purchasePrice;
			[1] call SOCK_fnc_updatePartial;
		...
		if (life_has_debit isEqualTo true) then {
			getInVehicles = getInVehicles - _purchasePrice;
...
if (LIFE_SETTINGS(getNumber,"player_advancedLog") isEqualTo 1) then {
    if (LIFE_SETTINGS(getNumber,"battlEye_friendlyLogging") isEqualTo 1) then {
        advanced_log = format [localize "STR_DL_AL_boughtVehicle_BEF",_className,[_purchasePrice] call life_fnc_numberText,[getInVehicles] call life_fnc_numberText,[getOutVehicle] call life_fnc_numberText];
    } else {
        advanced_log = format [localize "STR_DL_AL_boughtVehicle",profileName,(getPlayerUID player),_className,[_purchasePrice] call life_fnc_numberText,[getInVehicles] call life_fnc_numberText,[getOutVehicle] call life_fnc_numberText];
    };
